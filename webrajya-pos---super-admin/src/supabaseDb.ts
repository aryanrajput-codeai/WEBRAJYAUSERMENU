  ...
  try {
    ...
    const mappedRestaurants: Restaurant[] = rawRestaurants.map((r: any) => {
      ...
      return {
        ...
        email: r.owner_email || r.email || `owner_${r.slug || r.id.substring(0, 5)}@webrajyapos.com`,
        owner_email: r.owner_email || `owner_${r.slug || r.id.substring(0, 5)}@webrajyapos.com`,
        owner_password: r.owner_password || 'Admin@2026',
